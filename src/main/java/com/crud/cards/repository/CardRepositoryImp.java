...
public class CardRepositoryImp implements ICardRepository{
	...
	public int save(Cards card) {
		...
		return jdbcTemplate.update(SQL, new Object[] {card.getName(), card.getNumber(),
				card.getType(), card.getCvv(), card.getStatus(), card.getCreated_by(),
				card.getCreated_at()});
	}

	@Override
	public int update(Cards card) {
		String SQL = "UPDATE spring.cards SET name=?, number=?, type=?, cvv=? WHERE id_card = ?";
		return jdbcTemplate.update(SQL, new Object[] {card.getName(), card.getNumber(),
	...
	public int deleteByid(int id) {
		String SQL = "UPDATE spring.cards SET status = 0 WHERE id_card = ?";
